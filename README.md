...
> `POST /decisions` returns an opinion and the caller is what acts on it (`src/server.ts:8-14`).
...
set of actions and fail open, with an alert, on everything else** (`src/actions.ts:7-18`).

The split lives in `src/actions.ts` and not in the rules table, because **it is consulted precisely
when the rules table cannot be read** (`src/actions.ts:20-24`). A fail mode stored as a row is a
fail mode that is unavailable at the moment it is needed.

| Action | On store failure | Why |
| --- | --- | --- |
| `custody.key.export` | **deny** | the only action in the estate with no undo (`src/actions.ts:90-95`) |
| `ledger.treasury_spend` | **deny** | a spend from a platform account, not a user's (`src/actions.ts:107-111`) |
| `identity.session.new_device` | **deny** | the first step of every account takeover in the incident history; the availability cost is paid knowingly (`src/actions.ts:112-119`) |
| `wallet.withdrawal` | **deny at or above a floor**, allow below | SHARD 1000, EMBER 100, ETH 0.05, BTC 0.002, XRP 50 (`src/actions.ts:96-106`) |
| `wallet.deposit_address.assign`, `wallet.trusted_address.add`, `market.listing.create`, `trade.order.place`, `mint.deploy.request`, `identity.password.reset`, `api.request` | **allow, and alert** | each is a control whose value is statistical; blocking one request because a table blinked buys nothing (`src/actions.ts:121-165`) |

Three properties are worth knowing because they are easy to lose:

* **An unregistered action is a 400, never a guess** (`src/actions.ts:31-36`, `src/server.ts:643-656`).
...
* **`FAIL_CLOSED_ACTIONS` is derived from the registry, never written out** (`src/actions.ts:182-191`),
...
* **A withdrawal has two thresholds and they are different numbers on purpose.** The rule in the
  ...
  conservative (`src/actions.ts:26-29`). An asset with no floor is refused rather than defaulted
  (`src/actions.ts:261-264`) — a withdrawal in an asset nobody set a floor for is a withdrawal
  nobody has thought about.

A fail-open allow still carries the obligation `unchecked_decision`, `failOpen: true` is stored on
the decision row, and `policy_fail_open_total` increments (`src/decide.ts:60-84`). That counter
**must be zero**; a non-zero rate means decisions are being handed out with nothing behind them
(`src/server.ts:113-118`). Its twin `policy_fail_closed_total` is not an alert but a symptom: a
non-zero rate there means custody exports and treasury spends are being refused, and somebody is
about to report that the platform is broken when it is doing its job (`src/server.ts:99-103`).

## Routes

Read out of `src/server.ts:295-585`. Every route except the three probes calls `authenticate()`.
Every response carries `x-request-id` and `cache-control: no-store` — a cached decision is a
decision made against rules that have since changed (`src/server.ts:809-811`).

| Method | Path | Who | What it does |
| --- | --- | --- | --- |
| `GET` | `/livez` | public | static; a liveness probe that consults a dependency restarts a healthy process every time the database blinks (`src/server.ts:298-306`) |
| `GET` | `/readyz` | public | 503 until the Postgres probe passes (`src/server.ts:307-314`) |
| `GET` | `/metrics` | public | Prometheus text; gauges sampled at scrape time (`src/server.ts:315-332`) |
| `POST` | `/decisions` | **service only**, scope `policy:decide` | evaluate and record; 201 with the stored decision (`src/server.ts:336-358`) |
| `GET` | `/decisions/:id` | operator, `policy:decide`, **or the subject** | one decision (`src/server.ts:359-374`) |
| `GET` | `/subjects/:subject/decisions` | operator, `policy:decide`, or the subject | newest first; `?limit` 1–200, `?cursor` (`src/server.ts:375-395`) |
| `GET` | `/rules` | `role:admin` | the newest version of every rule, plus the action registry (`src/server.ts:399-409`) |
| `POST` | `/rules` | `role:admin` | writes the **next version**; 201 (`src/server.ts:410-433`) |
| `GET` | `/rules/:key` | `role:admin` | every version, newest first — where a decision's `key@version` citation is resolved months later (`src/server.ts:434-446`) |
| `DELETE` | `/rules/:key` | `role:admin` | disables by writing a new version. 200 with that version rather than 204: a delete here is an insert, and "no content" would hide that the history is still there (`src/server.ts:447-460`) |
| `POST` | `/trusted-addresses` | `role:admin` | 201; `coolingOffSeconds` defaults to 86 400, not 0 (`src/server.ts:464-492`) |
| `POST` | `/freezes` | `role:admin` | 201; 409 when a live freeze already covers this `(subject, scope)` (`src/server.ts:496-515`) |
| `GET` | `/freezes/:id` | `role:admin` | one freeze with its clearances (`src/server.ts:516-527`) |
| `GET` | `/subjects/:subject/freezes` | operator, `policy:decide`, or the subject | live freezes only (`src/server.ts:528-538`) |
| `DELETE` | `/freezes/:id` | `role:admin` | **202** on the first operator, 200 on the second (`src/server.ts:539-581`) |

A user token can never reach `POST /decisions`: `requireScope` is false for anything that is not a
service (`src/server.ts:341-343`). Deciding on your own behalf is not offered.

**A user may read a decision about themselves** (`src/server.ts:597-612`). That is not a courtesy.
...
**A verifier that cannot reach identity's JWKS answers 503, never 401** (`src/server.ts:273-278`).
...
that already knew to look (`src/server.ts:542-550`).

The asymmetry runs in the direction safety runs (`src/freezes.ts:4-12`). The mistake one operator
...
first. The operator who froze **may** be one of the two who clear (`src/freezes.ts:22-27`):
...
the fail-safe reachable from exactly one place (`src/decide.ts:4-8`).
...
1. **A freeze short-circuits everything.** It is checked before any rule and no rule can soften it:
   ...
   advisory (`src/evaluate.ts:179-193`). The score is reported as 100 rather than computed — a
   frozen subject is at maximum risk by definition, and computing one would invite a comparison
   that means nothing.
2. **Rules combine by taking the strictest, never by voting or averaging** (`src/rules.ts:32-48`).
...
3. **Risk bands are applied last**, because the score depends on whether a velocity rule breached,
   which is only known once every other rule has run (`src/evaluate.ts:273-297`).
...
and 5 per recent failure capped at 20 (`src/evaluate.ts:110-130`). It is a legible arithmetic an
...
**Reasons are codes, not sentences** (`src/evaluate.ts:17-22`) — `amount_at_or_above:1000:ETH`,
...
Parsed, never cast (`src/rules.ts:13-19`): a rule arrives as JSON on an admin route, and a
...
| --- | --- | --- |
| `amount_limit` | thresholds on one asset's amount | an empty list; thresholds are stored strictest-first so evaluation takes the first match and the stored rule is the evaluated one (`src/rules.ts:209-229`) |
| `velocity` | a cap per tumbling window, per subject and action | neither `maxCount` nor `maxAmount`; a `maxAmount` with no `asset`, because summing SHARD and ETH into one total is not a cap but a number (`src/rules.ts:231-249`) |
| `trusted_address` | a destination the subject has not sent to before | returns `hold_for_seconds:<n>` as an obligation rather than holding anything (`src/rules.ts:79-88`) |
| `cooling_off` | a named timer that must have elapsed | a timer key that is not lowercase (`src/rules.ts:264-275`) |
| `risk_score` | where a score stops being tolerable | bands out of order — an unreachable band reads as a control and is not one (`src/rules.ts:277-287`) |

The velocity window is a **tumbling** window keyed on the floor of the clock, and the trade is
stated rather than hidden (`src/rules.ts:60-67`): a calendar window lets a subject spend a full
...
already-recorded 3 against 3 would allow it (`src/evaluate.ts:217-227`).

A cooling-off timer is started by the **first refused attempt**, which is what makes it a period
rather than a formality (`src/evaluate.ts:253-262`): the first attempt is always refused and the
clock begins from that refusal.

**Not one comparison in this service goes through a float.** `compareDecimal` and `addDecimal`
split on the point and compare as padded integers (`src/actions.ts:197-217`,
`src/evaluate.ts:329-348`), and both return `null` for anything that is not a plain decimal, which
...
(`src/server.ts:666-673`).

## Background work

Two leased jobs and no timers. There is no `setInterval` in this repository and CI greps for one —
rule 8 (`src/jobs.ts:1-14`). **The lease key names the contended resource, not the row**: both jobs
...
| --- | --- | --- | --- |
| `policy.decisions.retention` | `global` | 1 h | one claims it; the other finds nothing (`src/jobs.ts:30-33`) |
| `policy.counters.prune` | `global` | 15 min | as above (`src/jobs.ts:30-33`) |

Retention deletes in batches of 1000 with a heartbeat between them, up to 100 passes
(`src/jobs.ts:78-104`). A single `DELETE` over two years of decisions takes a long transaction and
...
a moment later and the schedule would stop (`src/jobs.ts:42-51`). **A dead-lettered recurring job
...
(`src/migrations.ts:1-10`). `index.ts` asserts the version at boot and exits rather than serving,
so a replica of new code cannot answer requests against an old schema (`src/index.ts:54-63`).
...
| `jobs` | `@cloudsforge/jobs`, imported verbatim rather than hand-copied, so the claim query and the table cannot drift (`src/migrations.ts:33-41`) |

The constraints that carry meaning, and why each is in the schema rather than in a handler:

| Constraint | Refuses | Why here |
| --- | --- | --- |
| `policy_decisions_immutable` (trigger) | any `UPDATE` on a decision | evidence that can be edited is not evidence, and an edit would afterwards be indistinguishable from the decision having been made that way. The retention sweep still deletes, which is a different claim: the record is gone, not rewritten to say something it never said (`src/migrations.ts:105-118`) |
| `freeze_clearances` PK `(freeze_id, operator)` | the same operator clearing twice | a counter column would clear a freeze on one operator's double-click, and no amount of "check, then increment" fixes that under concurrency (`src/migrations.ts:212-225`) |
| `freezes_live_uniq` (partial, `where cleared_at is null`) | a second live freeze on one `(subject, scope)` | a duplicate would need clearing twice over; the insert answers 409 rather than returning somebody else's freeze with the wrong reason attached (`src/migrations.ts:204-208`, `src/freezes.ts:120-126`) |
| `trusted_addresses_live_uniq` (partial, `where revoked_at is null`) | two live entries for one destination | partial, so a revoked entry does not block re-adding an address later — which users legitimately do after losing a wallet (`src/migrations.ts:164-168`) |
| `velocity_counters` PK including `asset`, defaulted to `''` and never null | two rows for "no asset", each holding half the count | a null in a primary key does not compare equal to itself (`src/migrations.ts:133-139`) |
| `policy_rules_key_version_uniq` | a second row at one `(key, version)` | the version is computed inside the insert, so two operators editing at once produce 4 and 5 rather than both producing 4 and one reporting success it did not have (`src/store.ts:269-292`) |
| `policy_decisions_verdict`, `policy_decisions_risk_range` | a verdict outside the four, a score outside 0–100 | the wire vocabulary and the schema cannot drift apart (`src/migrations.ts:93-94`) |

Two writes are load-bearing beyond their constraints. **Only an `allow` consumes velocity budget**
(`src/store.ts:182-192`): a denied request did not happen, and counting it would let an
...
than the database's default** (`src/store.ts:221-247`): if a repeated attempt restarted the clock a
...
(`src/decide.ts:10-13`, `src/decide.ts:98-108`). A decision recorded without its counter lets a
...
(`src/decisions.ts:7-12`): what was actually consulted, and nothing else. A verbatim copy of the
...
container the estate's secrets (`src/env.ts:1-12`). Validation runs at import, so a bad value exits
...
report of it (`src/env.ts:128-150`).

| Variable | Default | If it is wrong |
| --- | --- | --- |
| `POLICY_DATABASE_URL` | — | refuses to start (`src/env.ts:116`). Rule 1: one database, and CI fails the build on a second connection string |
| `IDENTITY_JWKS_URL` | — | refuses to start (`src/env.ts:120`); also a **soft** readiness probe, because marking it hard would remove every service in the estate from its balancer on one identity blip (`src/index.ts:89-94`) |
| `IDENTITY_ISSUER` | — | refuses to start (`src/env.ts:121`) |
| `PORT` | `4000` | (`src/env.ts:112`) |
| `LOG_LEVEL` | `info` | a value outside debug/info/warn/error refuses to start (`src/env.ts:107-110`) |
| `POLICY_DATABASE_POOL_MAX` | `10` (1–100) | a pool larger than the database's budget divided by the replica count exhausts Postgres for everything else (`src/env.ts:117-119`) |
| `POLICY_DECISION_RETENTION_DAYS` | `730` (**floor 90**) | the floor refuses a value that would delete the evidence before the argument about it has started (`src/env.ts:81-89`, `src/env.ts:123`) |
| `POLICY_COUNTER_RETENTION_HOURS` | `48` (2–720) | too low, and a late decision inside a still-open window finds no bucket (`src/env.ts:90-97`) |
| `INSTANCE_ID` | hostname | names this replica in `jobs.locked_by` — the container id under compose, the pod name under Kubernetes (`src/env.ts:75-80`) |
| `CLOUDSFORGE_TAG` | `dev` | reported on every log line and in the release manifest (`src/env.ts:114`) |

There is no `requiredSecret` helper and no `OUTBOX_SIGNING_SECRET`, and both absences are
deliberate (`src/env.ts:31-38`). **This service holds no secret at all**: it verifies tokens against
...
rule 9 it is not declared (`src/index.ts:10-13`).

## What it talks to

| Upstream | What for | When it is down |
| --- | --- | --- |
| `micro-identity` | JWKS, to verify bearer tokens. Nothing else | 503 on requests needing a fresh key; readiness stays green, because the probe is soft (`src/index.ts:89-94`, `src/server.ts:273-278`) |
...
* **No outbox, no inbox, no events.** The template ships an outbox and most services need one, but
  ...
  this service is entitled to publish (`src/migrations.ts:12-26`). The trigger for adding one is
...
* **No rule is ever deleted.** `DELETE /rules/:key` writes a disabling version
  (`src/store.ts:318-338`). A deleted rule takes its history with it, and every decision citing it
  ...
  decision that cited it unreproducible (`src/rules.ts:4-11`).
...
is the difference between a red build and an emptied environment (`src/testsupport.ts:4-6`).
...
`failingReader()` (`src/testsupport.ts:81-93`) is the seam that makes AD-09's split testable at
...
* **A velocity budget is consumed by an allowed decision the caller may never act on.** Named at
  `src/store.ts:186-192`, with the reason the alternative is worse.
* **A trusted address is matched exactly and deliberately not case-folded**
  (`src/store.ts:162-167`). An EVM address is case-insensitive but a Bitcoin or Solana base58
